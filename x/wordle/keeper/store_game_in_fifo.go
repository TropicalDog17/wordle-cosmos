...
func (k Keeper) SendToFifoTail(ctx sdk.Context, game *types.Game, info *types.SystemInfo) {
	if info.FifoHeadIndex == types.NoFifoIndex && info.FifoTailIndex == types.NoFifoIndex {
		...
		// Now add to tail
		currentTail, found := k.GetGame(ctx, info.FifoTailIndex)
		if !found {
			panic("Current Fifo tail was not found")
		}
		currentTail.AfterIndex = game.Index
		k.SetGame(ctx, currentTail)
